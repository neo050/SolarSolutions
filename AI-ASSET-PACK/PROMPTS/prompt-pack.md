...
**Filename to return:** `builders-hero-shell-stage.webp`
**Aspect ratio:** 16:9 · **Resolution:** 2400×1350

**Reference images to use (attached):**
  - SRC-SITE-001 — אתר בנייה למגורים בשלב שלד — צנרת, שרוולים, קופסאות, הכנות לפני יציקה.
  - SRC-COMMERCIAL-001 — פרויקט מסחרי בשלב בנייה: תקרת גבס פתוחה עם פתחים עגולים לגופי תאורה, צנרת שרשורית, מוליכים משוכים ומחכים, סולם
  - SRC-PANEL-005 — לוח מסחרי גדול בן 72 מודולים בארבע שורות — Hager, בקר בידוד ISO-2, מאמתים תלת-פאזיים, מגענים Eaton, ומספור מלא

**המפרט (לקריאה שלך):** תשתית חשמל באתר בנייה למגורים לפני יציקה · שלד בטון, קירות בלוקים, צנרת שרשורית וקופסאות מותקנות · צנרת מובילה את העין לעומק הפריים
...
**Filename to return:** `texture-copper-busbar.webp`
**Aspect ratio:** 21:9 · **Resolution:** 2560×1097

**Reference images to use (attached):**
  - SRC-PANEL-005 — לוח מסחרי גדול בן 72 מודולים בארבע שורות — Hager, בקר בידוד ISO-2, מאמתים תלת-פאזיים, מגענים Eaton, ומספור מלא
  - SRC-PANEL-001 — לוח חשמל ביתי פתוח, שורת מאמתים CHINT, ממסר פחת, ותוויות מודפסות בעברית לכל מעגל (מזגן, כירה, דוד, מטבח, ראשי)

**המפרט (לקריאה שלך):** מקרו מופשט של פסי צבירה מנחושת ומוליכים, מטושטש מאוד · מקרו סטודיו · מרקם אחיד ללא נקודת מוקד
...
> Match the reference images for equipment type, clothing and environment.
> **Framing constraint (this is not optional — it fills a fixed slot in a built
> layout): The entire frame carries text on top, so keep it uniform with no sharp detail anywhere.**
> Negative: no text, no logos, no brand names, no phone numbers, no readable signage,
> no identifiable faces other than those in the references, no over-saturated colour,
> no plastic skin, no impossible wiring, no unsafe practice, no missing PPE.
...
**Filename to return:** `og-contracting.png`
**Aspect ratio:** 1.91:1 · **Resolution:** 1200×630

**Reference images to use (attached):**
  - SRC-PANEL-005 — לוח מסחרי גדול בן 72 מודולים בארבע שורות — Hager, בקר בידוד ISO-2, מאמתים תלת-פאזיים, מגענים Eaton, ומספור מלא

**המפרט (לקריאה שלך):** לוח מסחרי גדול, עם שכבת כהות ומקום לכותרת · חדר חשמל או קיר גמר · הלוח בצד ימין, שני שלישים שמאליים כהים לטקסט
...
> Match the reference images for equipment type, clothing and environment.
> **Framing constraint (this is not optional — it fills a fixed slot in a built
> layout): Social platforms crop this to a square from the centre, so the subject must sit centrally. Keep the left two thirds dark and empty for a headline.**
> Negative: no text, no logos, no brand names, no phone numbers, no readable signage,
> no identifiable faces other than those in the references, no over-saturated colour,
> no plastic skin, no impossible wiring, no unsafe practice, no missing PPE.
...
**Filename to return:** `og-solar.png`
**Aspect ratio:** 1.91:1 · **Resolution:** 1200×630
...
**המפרט (לקריאה שלך):** שורות פאנלים על גג, שמיים נקיים, שכבת כהות לטקסט · גג בית פרטי או מבנה חקלאי · פאנלים באלכסון מהפינה הימנית התחתונה
...
> Match the reference images for equipment type, clothing and environment.
> **Framing constraint (this is not optional — it fills a fixed slot in a built
> layout): Social platforms crop this to a square from the centre — keep the centre uncluttered.**
> Negative: no text, no logos, no brand names, no phone numbers, no readable signage,
> no identifiable faces other than those in the references, no over-saturated colour,
> no plastic skin, no impossible wiring, no unsafe practice, no missing PPE.
...
**Filename to return:** `ev-charger-install.webp`
**Aspect ratio:** 3:2 · **Resolution:** 1800×1200

**Reference images to use (attached):**
  - SRC-PANEL-001 — לוח חשמל ביתי פתוח, שורת מאמתים CHINT, ממסר פחת, ותוויות מודפסות בעברית לכל מעגל (מזגן, כירה, דוד, מטבח, ראשי)
  - SRC-PANEL-004 — לוח דירתי עם מונה NISKO, פחת ראשי, מפסק ראשי ופיצול פאזות (L3+0). תיוג מודפס בעברית לכל רכיב, כולל 'מונה דירה 

**המפרט (לקריאה שלך):** עמדת טעינה ביתית מותקנת על קיר חניה, כבל מסודר · חניה פרטית או חניון תת-קרקעי · העמדה בשליש הימני, קיר נקי משמאל
...
> Match the reference images for equipment type, clothing and environment.
> **Framing constraint (this is not optional — it fills a fixed slot in a built
> layout): No text is set over this one; it is ordinary body imagery. Keep the left half plain.**
> Negative: no text, no logos, no brand names, no phone numbers, no readable signage,
> no identifiable faces other than those in the references, no over-saturated colour,
> no plastic skin, no impossible wiring, no unsafe practice, no missing PPE.
